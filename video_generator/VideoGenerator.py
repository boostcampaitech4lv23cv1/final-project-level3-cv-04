...
def video_df(df1,pred,member):  # 들어온 df를 깔끔하게 정리
    ...
    df1 = df1.drop('track_conf',axis=1)
    trackID_by_member = []
...
def find_center_in_image(mem_idx, idx, df,member):
    if len(mem_idx) > 1:
        ...
        body_xmax = float(df[df['filename'] =='{0:06}.jpg'.format(idx) ]['track_body_xmax'].values[0])
        body_ymax = float(df[df['filename'] =='{0:06}.jpg'.format(idx) ]['track_body_ymax'].values[0])
        body_center_x = (body_xmax - body_xmin)/2 + body_xmin
        body_center_y = (body_ymax - body_ymin)/10 + body_ymin
        distances = []
        centers_x = []
        centers_y = []
        face_keypoints  = list(df['face_keypoint'][(df['filename']=='{0:06}.jpg'.format(idx))])
        for i in mem_idx:
        ...
        _series = df[df['filename'] == '{0:06d}.jpg'.format(idx)].iloc[0]
...
def coordinate_for_crop_img(center_x,center_y,video_size_w,video_size_h,face_loc):   
