...
우리는 hard augmentation이 적용된 Crowdhuman dataset에 트레인된 모델의 가중치를 soft-augmentation으로 변경하여 재 트레이닝 하였습니다.  
tracking 모델만의 결과는, 모델 자체의 한계로 인해 동일인에 대하여 다수의 id가 부여됩니다.

### 2) Postprocesser

Postprocessor는 세가지 역할을 수행합니다.  
첫번째, 특정 id의 bbox 겹침이 심할 경우 새로운 아이디를 부여합니다.  
